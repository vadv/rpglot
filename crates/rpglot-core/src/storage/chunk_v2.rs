//! New storage format with per-snapshot zstd frames and O(1) random access.
//!
//! File layout:
//! ```text
//! ┌─────────────────────────────────────────────────────────┐
...
//! │   _reserved: [u8; 4]          = [0; 4]                  │
//! ├─────────────────────────────────────────────────────────┤
//! │ INDEX TABLE (snapshot_count × 16 bytes, uncompressed)   │
//! │   Per snapshot:                                         │
...
const INDEX_ENTRY_SIZE: usize = 16; // 2 × u64

/// Reader for new-format chunk files with per-snapshot random access.
pub struct ChunkReader {
    snapshot_count: usize,
    /// (byte_offset, compressed_len) for each snapshot frame.
    index: Vec<(u64, u64)>,
    interner_offset: u64,
    interner_compressed_len: u64,
...
impl ChunkReader {
    ...
    pub fn open(path: &Path) -> io::Result<Self> {
        ...
        for i in 0..snapshot_count {
            ...
            index.push((offset, compressed_len));
        }
    ...
    pub fn read_snapshot(&self, idx: usize) -> io::Result<Snapshot> {
        ...
        let (offset, compressed_len) = self.index[idx];
...
pub fn write_chunk(
    ...
    // Write snapshot frames, recording offsets
    let mut index_entries: Vec<(u64, u64)> = Vec::with_capacity(snapshot_count as usize);
    ...
    for snapshot in snapshots {
        ...
        index_entries.push((offset, compressed.len() as u64));
    }
    ...
    // Write real index
    for (offset, compressed_len) in &index_entries {
...
/// Checks if a file starts with the RPG2 magic bytes.
pub fn is_v2_format(path: &Path) -> io::Result<bool> {
...
mod tests {
    ...
    fn test_is_v2_format() {
        ...
        // Write v2 file
        let v2_path = dir.path().join("v2.zst");
        let snapshots = create_test_snapshots(1);
        write_chunk(&v2_path, &snapshots, &StringInterner::new()).unwrap();
        assert!(is_v2_format(&v2_path).unwrap());

        // Write non-v2 file (raw zstd)
        let old_path = dir.path().join("old.zst");
        let raw = b"hello";
        let compressed = zstd::encode_all(&raw[..], 3).unwrap();
        std::fs::write(&old_path, &compressed).unwrap();
        assert!(!is_v2_format(&old_path).unwrap());
    }
}
